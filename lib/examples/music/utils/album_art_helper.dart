...
import '../service/musicserver_nokey/music_info_service.dart';
import '../service/musicserver_nokey/music_service_locator.dart';

/// 根据「艺人 + 歌曲名」获取歌曲封面的本地缓存路径。
///
/// 调用流程：
/// 1. 计算缓存目录和缓存文件名（基于 artist + title）
/// 2. 如果本地已存在缓存图片，直接返回该文件路径；
/// 3. 如果不存在：
///    - 调用 music_info_service 获取综合信息；
///    - 如果返回了 coverUrl，则下载并写入缓存文件；
/// 4. 任意环节失败则返回 null（调用方据此决定用占位图等）。
Future<String?> getAlbumArtPath(String artist, String title) async {
  try {
    ...
    // 2. 基于「艺人 + 歌曲名」生成一个相对安全的文件名
    //    曲目名里可能包含 / \ : * ? " < > | 这些在部分文件系统是非法的，所以需要做简单过滤。
    final String safeFileName =
        _sanitizeFileName('$artist - $title', suffix: '.jpg');
    ...
    // 4. 本地无缓存，向 MusicBrainz/CoverArtArchive 请求歌曲封面 URL
    final FullTrackInfo? fullTrackInfo =
        await musicInfoService.fetchFullTrackInfo(artist, title);
    final String? albumArtUrl = fullTrackInfo?.coverUrl;
